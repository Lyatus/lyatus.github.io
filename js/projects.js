var projects = {
	'l-engine':{
		'title':'L Engine',
		'short':[
			"Personal C++/OpenGL engine with minimal dependencies (as in little to no STL) for Windows and Unix",
		...
		'content':
			...
			+"<h2>Rendering</h2><p>The rendering pipeline is still very young but I'm pretty confident I want to keep it fully deferred. My G-buffer for now is made up of diffuse+spec on 4 bytes, normals on 3 16-bit floats, and the 24-bit depth buffer is used to reconstruct position with the inverse view*projection matrix. My ambition is to fake transparency using dithering, you can see the first attempt with a 2x2 Bayer matrix on the right, but I'm obviously planning on letting the choice of matrix to the user, with a void and cluster matrix as default. It still needs good anti-aliasing to look decent too.</p>"
			+"<h2>Physics</h2><p>The collision system uses the separating axis theorem but only supports boxes for the moment and is lacking a broadphase (I'm planning on using AABB dynamic tree). The physics engine does not yet support friction and resting contacts. The collision system fires events to scripts on the same entity to allow custom reactions.</p>"
			+"<h2>Scripting</h2><p>The scripting language has its syntax inspired from Lisp (parentheses everywhere), and its core design inspired by Lua (simple yet extendable). The choice for the Lisp syntax is because I had been struggling to do decent parsing in earlier versions of the language, and I figured it'd be nice to have the simplicity in design of Lua combined with the barest syntax. The virtual machine is very simple and inefficient for the moment, and memory fragmentation is very high, these are things I'm often thinking about but will probably solve when script performance becomes an issue. I've been experimenting with dynamic assembly too (in x86), and I'm not sure it's the right way to go but it would be a interesting exercise to make a JIT compiler for the language. I'm trying to frequently update a GitHub <a href=\"https://gist.github.com/Lyatus/2800c45904af8d8c63f821d0b8924a8f\">gist</a> as a script reference but it's incomplete.</p>"
			+"<h2>Threading</h2><p>There's still no threading in the engine but I've started doing some experiments with fibers, I was very much influenced by Christian Gyrling's <a href=\"http://www.gdcvault.com/play/1022186/Parallelizing-the-Naughty-Dog-Engine\">talk</a> about Naughty Dog's use for them in their job system. I'm always trying to keep threading in mind but I know there will be surprises when I finally make use of a job system.</p>"
			+"<h2>Sources</h2><p>I intend to make the sources available at some point but there are still some embarassing pieces of code that need to be rewritten before that. I also feel like making the sources available is an action that I cannot take back and it's hard for me to take a decision with confidence. On an other note, the release of the binaries and documentation to use the engine is a no-brainer, I will put direct download links here as soon as I consider the engine to be \"public ready\" (that is when the currently being developed proof-of-engine game is done).</p>",
